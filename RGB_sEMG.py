...
def init_operations():
    ...
    if args.wandb_name is not None:
        ...
        wandb.run.name = f'RGB-sEMG'

"""
TODO: ADD to yaml file
args.train.in_feature_size, 
...
def reconstruct(autoencoder, dataloader, device, split=None, **kwargs):
    ...
    try:
        ...
        if not os.path.isdir(os.path.join('./saved_features/translated/', date)):
            os.mkdir(os.path.join('./saved_features/translated/', date))
        with open(os.path.join('./saved_features/translated/', date, f"{filename}_{args.dataset.shift.split('-')[0]}_{split}.pkl"), "wb") as file:
