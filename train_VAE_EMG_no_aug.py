...
def init_operations():
    ...
    if args.wandb_name is not None:
        ...
        run = wandb.init(project="FC-VAE(emg)", entity="egovision-aml22", name = f"{args.models.EMG.model}_{m}_{args.models.EMG.lr}")
...
def validate(autoencoder, val_dataloader, device, reconstruction_loss):
    ...
    return total_loss/(10 * len(val_dataloader))
...
def save_model(model, filename):
        try:
            ...
            filename = os.path.join('./saved_models/VAE_EMG', date, f"{filename}_{ts}.pth")
